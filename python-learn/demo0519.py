...
sub内部再调用test方法时，会把每一个匹配到的数据以re.Match的格式传参
re.sub(r'\d', 'x', 'aaa111ss11aa11')
def test(x):
    return x * 2
re.sub(r'\d+', test, 'aaa111ss11aa11')
...
"""
贪婪模式和非贪婪模式
在python默认贪婪模式 尽可能多的匹配
在贪婪模式后面添加? 可以将贪婪模式转换成为非贪婪模式
m = re.rearch(r'm.*a', 'addsmsdfa')
m = re.rearch(r'm.*?a', 'addsmsdfa') 非贪婪模式 尽可能少的匹配
"""

"""
网络通信概念
网络时用物理链路将各个孤立的工作站或主机相连在一起，组成数据链路，从而达到资源共享和通信的目的

ip地址要在同一网段才能通信 ip地址是使用八个二进制 255
IP地址：192.168.31.100
子网掩码255.255.255.0 按位与运算
网段：192.168.31.X ==>x 表示的是主机位 这个网络；里最多放多少台主机 254  
192.168.x 主机位全0表示网络位 192.168.1.0 主机位全1表示广播 192.168.1.255

ip地址 点分十进制 
每一位都是使用八个二进制 255.255.255.0

ip地址分类
A类地址：01111111 11111111 11111111 11111111  使用七位表示网络位
子网掩码： 11111111 00000000 00000000 00000000 255.0.0.0
A类地址 0.0.0.0 => 126.255.255.254 ===> 127开头得失特殊的ip地址，0.0.0.0都是特殊的ip地址
